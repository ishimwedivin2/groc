...
public class AdminUserController {
    ...
    public ResponseEntity<ByteArrayResource> downloadUsers() throws IOException {
        ...
        // Write CSV header to match the upload format
        writer.println("Username,First Name,Last Name,Email,Phone Number,Profile Picture,Role");

        // Write user data to match the upload format
        for (User user : users) {
            writer.printf("%s,%s,%s,%s,%s,%s,%s%n",
                    ...
                    user.getRole().name()); // assuming getRole() returns an enum
        }
        writer.flush();
        writer.close();
